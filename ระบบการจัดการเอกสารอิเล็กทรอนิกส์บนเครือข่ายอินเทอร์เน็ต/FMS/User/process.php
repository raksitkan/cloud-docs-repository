...
if(isset($_REQUEST['btn_insert']))
{
	try
	{
		...
		if($image_file)
		{
			if($allowedfileExtensions = array('zip', 'txt', 'xls', 'doc','pdf','rar','ppt','pptx','xls','xlse','xlw','docx')) //check file extension
			{	
       ...
       $insert_stmt->bindParam(':subjects',$name);
            ...
            $insert_stmt=$conn->prepare('INSERT INTO file_tbl(fname,file_name,file_name_random,m_username,status,host,ip,file_type) VALUES(:fname,:ffile,:rfileup,:username,:status,:hostup,:ipup,:file_type)'); //sql insert query					
			$insert_stmt->bindParam(':fname',$name);
            ...
            $insert_stmt->bindParam(':hostup',$host);
            $insert_stmt->bindParam(':ipup',$ip_user);
            $insert_stmt->bindParam(':file_type',$ext);//เพิ่ม type
