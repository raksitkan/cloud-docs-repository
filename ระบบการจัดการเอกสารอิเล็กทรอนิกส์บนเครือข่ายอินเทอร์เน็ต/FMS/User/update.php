            ...
            <div class="col">
				...
				if(isset($_REQUEST['btn_update']))
				{
					try
					{
						...
						if($name!='')
						{
							$update_stmt=$conn->prepare('UPDATE file_tbl SET name=:name_up WHERE id=:id'); //sql update query
					   ...
					   $insert_stmt->execute();
						...
						if(!isset($errorMsg))
						{
							$status ='Update file successfully';
							$insert_stmt=$conn->prepare('INSERT INTO file_log(file_name,name_random,action,create_by,ip,host) VALUES(:fileup,:filerandom,:actionup,:m_username,:ipup,:hostup)'); //sql insert query					
							//bind all parameter 
					   $insert_stmt->bindParam(':fileup',$raw_file);
					   $insert_stmt->bindParam(':filerandom',$image_file);
					   ...
					   $insert_stmt->bindParam(':hostup',$host);
							...
							$update_stmt=$conn->prepare('UPDATE file_tbl SET name=:name_up,file_name=:raw_name,file=:file_up ,ip=:ip_up,status=:status,host=:hostup WHERE id=:id'); //sql update query
							$update_stmt->bindParam(':name_up',$name);
							...
							if($update_stmt->execute())
							{
								...
								header("refresh:2;user.php");	//refresh 3 second and redirect to index.php page
							}
						}
					}
					catch(PDOException $e)
		...
		?>
                <form method="post" class="form-horizontal" enctype="multipart/form-data">
                    <div class="form-group">
                        ...
                        <!-- <input type="text" name="txt_name" class="form-control" value="<?=$row["name"]?> " /> -->
