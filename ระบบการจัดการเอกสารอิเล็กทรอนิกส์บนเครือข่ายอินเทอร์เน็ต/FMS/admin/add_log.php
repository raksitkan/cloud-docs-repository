...
<br><br>
  ...
  <ol class="breadcrumb">
    <li class="breadcrumb-item " aria-current="page">ข้อมูลการจัดการข้อมูลผุ้ใช้</li>
  </ol>
</nav>
<?php
    ...
    foreach($_POST["search"] as $k=>$v){
        if(!empty($v)) {
            ...
            switch($k) {
                ...
                case "timestemp":
                    $create_time = $v;
                    $queryCondition .= "and time_add LIKE '" . $v . "%'";
                    break;
            }
        }
    }
...
$sql = " SELECT *, DATE_FORMAT(time_add, '%d/%m/%Y/%T') as dt FROM add_user_log where log_id " . $queryCondition ;//ดึงขอมูลเฉพาะuserที่เป็นคนอัพโหลดไฟล์ 
...
?>
                <form name="frmSearch" method="post" action="add_log.php">
...
<div class="date-selector">
  ...
  <span id="datePickerLbl" style="pointer-events: none;"></span>
                        ...
                        <div class="form-group mr-2">
                        <input type="reset" class="btn btn-secondary" value="Reset" onclick="window.location='add_log.php'">
                        </div>
                    </div>
					...
					?>
                            <tr>
                                ...
                                <td><?php echo $result[$k]["m_username"]; ?></td>
                                <td><?php echo $result[$k]["m_password"]; ?></td>
                                <td><?php echo $result[$k]["m_action"]; ?></td>
                                <td><?php echo $result[$k]["action_by"]; ?></td>
    ...
    <br>
                    ...
                    <div class="modal-footer d-flex justify-content-center">
                        <button id="btn_insert" name="btn_add_log" class="btn btn-info">Submit <i
